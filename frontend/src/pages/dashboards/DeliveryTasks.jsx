      ...
      {!hasActiveTasks ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {tasks.orders.map(order => (
            <TicketCard key={order.id} className="bg-chalk shadow-sm border-ink/10 p-4 hover:-translate-y-1 transition-transform duration-200 flex flex-col justify-between">
              <div>
              ...
              <div className="mt-4 pt-4 border-t border-ink/10">
                 {order.status === 'PENDING' || order.status === 'READY_FOR_PICKUP' ? (
                     ...
                     <div className="flex flex-col items-center">
                         ...
                         {updating === order.id ? (
                            <div className="flex justify-center p-2"><Loader2 className="w-6 h-6 animate-spin text-bazaar-green" /></div>
                         ) : (
                            <ImageUploader 
                     ...
                     <div className="bg-bazaar-green/10 text-bazaar-green text-center py-2 rounded-lg font-bold text-sm">
                         Delivery Completed
                     </div>
                 ) : (
