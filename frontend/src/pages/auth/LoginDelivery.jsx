...
const LoginDelivery = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      if (!email || !otp) {
        setError('Email address and OTP code are required');
        setLoading(false);
        return;
      }
      ...
      if (data.role === 'DELIVERY_PARTNER') {
        ...
        setError('Unauthorized access. This portal is for Delivery Partners only.');
        setLoading(false);
      }
    } catch (e) {
      setError(e.response?.data?.message || 'Invalid email or OTP code');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="mt-2 text-sm text-gray-500 dark:text-gray-400 font-medium">
          {t('auth.deliveryLoginSubtitle') || 'Sign in to view delivery tasks'}
        </p>
      </div>
      ...
      <form className="space-y-5" onSubmit={handleLogin}>
        ...
        <p className="text-center text-sm text-gray-500 dark:text-gray-400 font-medium">
          ...
          <Link to="/register/delivery" className="font-extrabold text-emerald-600 hover:text-emerald-500 dark:text-emerald-400 dark:hover:text-emerald-350">
            Register
          </Link>
        </p>
